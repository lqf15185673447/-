...
#define IF_NAME "enp2s0" // just ifconfig to see what ur name is and get from there....
                         // u can query it from system, but i was lazy experimentor.

// #define Packet_num 400*136*1440*7      //包发送的个数 400*136=1min   week
// #define Packet_num 1800*100      //包发送的个数       1800*100 
// #define Packet_num 400*8160*2      //包发送的个数 
// #define Packet_num 100      //包发送的个数 
#define Packet_num 15000*3600*39      //包发送的个数 

#define tx_len1  0
#define ETHER_TYPE1 0x9208     //0x9208   0x0008  0x0608     0x8808

#define SEQ_OFFSET 14

void Sleep_Ms(uint16_t ms);
void Sleep_Us(uint16_t us);
void Sleep_Ns(uint16_t ns);
...
int main(int argc, char **arg_list)
{
      ...
      saddr.sll_hatype = ARPHRD_ETHER; // these values basically correspond with the socket type u made....
      ...
      {                                          // ioctl is a bit of a beast, but basically you ask driver for info
      ...
      // where to send...
   //   unsigned char dest_mac[ETH_ALEN] = {0xa0, 0x36, 0x9f, 0xcd, 0xb4, 0xbf}; // enp2s0
      //  unsigned char dest_mac[ETH_ALEN] = {0xa0, 0x36, 0x9f, 0xcd, 0xb4, 0xbf}; 
      unsigned char dest_mac[ETH_ALEN] = {0x60, 0xbe, 0xb4, 0x03, 0x06, 0x0e}; //f3 enp2s0
      memcpy(&ehdr->ether_dhost, dest_mac, ETH_ALEN);

//      ehdr->ether_type = 0x0008; // u set next protocol type here. for example u could htons(ETH_P_IP) to set to IPv4 type...
      ehdr->ether_type = ETHER_TYPE1; // u set next protocol type here. for example u could htons(ETH_P_IP) to set to IPv4 type...

      int tx_len = tx_len1;                        // size we want to send...
      ...
      for (uint64_t i = 0; i < Packet_num; i++)
      {
            ...
            memcpy(sendbuf + SEQ_OFFSET, &seq_num, sizeof(seq_num));
            seq_num++;
